...
class pipeline:
    ...
    @classmethod
    def aum_run(cls, config, seed, fakedata_indices, train, dev, threshold_set_no, noise):
        ...
        train_dataset = Data.get_dataset(train, config["model"], config["dataset"], True, fakedata_indices,
                                         fake_label)
        test_dataset = Data.get_dataset(dev, config["model"], config["dataset"], True, fakedata_indices,
                                         fake_label)
        # model
        ...
        model = Execution.training(model, bsize, n_epochs, train_dataset, test_dataset, lrate, optimizer, True, save_dir,
                                   aum_calculator)
    ...
    def run(cls, config, seed, noise):
        ...
        # model = Execution.training(model, bsize, n_epochs, train_dataset,dev_dataset , lrate, optimizer, False,evaluate=True)
        ...
        # for i in classes:
        fakedata_indices_1=fakedata_indices_1.union(set(random.sample(list(set(range(N))), fakedata_count)))
        temp = list(set(range(N)) - set(fakedata_indices_1))
        # temp = i - fakedata_indices_1
        fakedata_indices_2=fakedata_indices_2.union(set(random.sample(temp, fakedata_count)))

        bad_data1 = cls.aum_run(config, seed, fakedata_indices_1, train, dev, "1", noise)
        bad_data2 = cls.aum_run(config, seed, fakedata_indices_2, train, dev, "2", noise)
        ...
        model = Execution.training(model, bsize, n_epochs, train_dataset_clean, dev_dataset, lrate, optimizer, False,evaluate=True)
