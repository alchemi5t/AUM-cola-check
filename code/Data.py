...
class Data:
    ...
    def get_data(cls, dataset_name, dataset_dir, data_file_type, data_file_sep, noise = None):
        if(dataset_name == 'CoLA'):
          raw_train = pd.read_csv("/content/AUM-main/code/CoLA/train.tsv",sep='\t', header=None, names=["f1","label","f2","sentence"])
          train = Dataset.from_pandas(raw_train[['sentence','label']])
       
          raw_dev = pd.read_csv("/content/AUM-main/code/CoLA/dev.tsv",sep='\t', header=None, names=["f1","label","f2","sentence"])
          dev = Dataset.from_pandas(raw_dev[['sentence','label']])
        else:
    ...
    @classmethod
    def distilbert_CoLA_AUM(cls, data, fake_index, fake_label):
        if not cls.dbtokenizer:
            cls.dbtokenizer = DistilBertTokenizerFast.from_pretrained('distilbert-base-uncased')
        enc = cls.dbtokenizer(data['sentence'], truncation=True, padding=True, max_length=128)
        dataset = SST2_AUM(enc, (torch.Tensor(data['label'])).long(), fake_index, fake_label)
